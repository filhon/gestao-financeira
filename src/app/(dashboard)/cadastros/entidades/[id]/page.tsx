...
export default function EntityDashboard() {
    ...
    const { canViewEntities } = usePermissions();
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                const allTxs = await transactionService.getAll({ companyId: selectedCompany.id });

                // Filter transactions associated with this entity
                // Match by entityId OR by supplierOrClient name (for legacy/manual entries)
                const entityTxs = allTxs.filter(tx =>
                    tx.entityId === id ||
                    (tx.supplierOrClient && entityData.name &&
                        tx.supplierOrClient.toLowerCase() === entityData.name.toLowerCase())
                );
    ...
    if (!entity) {
        return <div className="flex items-center justify-center h-screen">Entidade não encontrada.</div>;
    }
    ...
    // Calculations
    const payables = yearTransactions.filter(t => t.type === 'payable' && t.status !== 'rejected');
    const receivables = yearTransactions.filter(t => t.type === 'receivable' && t.status !== 'rejected');

    const totalPayables = payables.reduce((acc, t) => acc + t.amount, 0);
    const totalReceivables = receivables.reduce((acc, t) => acc + t.amount, 0);

    const paidPayables = payables.filter(t => t.status === 'paid').reduce((acc, t) => acc + (t.finalAmount || t.amount), 0);
    const paidReceivables = receivables.filter(t => t.status === 'paid').reduce((acc, t) => acc + (t.finalAmount || t.amount), 0);
    ...
    const statusDistribution = [
        { name: "Pago", value: payables.filter(t => t.status === 'paid').length + receivables.filter(t => t.status === 'paid').length, color: "#22c55e" },
        { name: "Aprovado", value: payables.filter(t => t.status === 'approved').length + receivables.filter(t => t.status === 'approved').length, color: "#3b82f6" },
        { name: "Pendente", value: payables.filter(t => t.status === 'pending_approval').length + receivables.filter(t => t.status === 'pending_approval').length, color: "#f59e0b" },
        { name: "Rascunho", value: payables.filter(t => t.status === 'draft').length + receivables.filter(t => t.status === 'draft').length, color: "#6b7280" },
    ].filter(d => d.value > 0);
    ...
    const monthlyTrendData = yearTransactions
        .reduce((acc, t) => {
            ...
            return acc;
        }, [] as { name: string; monthIndex: number; payables: number; receivables: number }[])
    ...
    const getCategoryBadge = (category: string) => {
        switch (category) {
            case "supplier": return <Badge variant="outline" className="border-red-500 text-red-600">Fornecedor</Badge>;
            case "client": return <Badge variant="outline" className="border-green-500 text-green-600">Cliente</Badge>;
            case "both": return <Badge variant="outline" className="border-purple-500 text-purple-600">Fornecedor/Cliente</Badge>;
