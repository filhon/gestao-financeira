        ...
        const loadDashboard = async () => {
            ...
            try {
                const [metricsData, transactions] = await Promise.all([
                    ...
                    transactionService.getAll({ companyId: selectedCompany.id })
                ]);
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {upcomingTransactions.length === 0 ? (
                                <p className="text-sm text-muted-foreground text-center py-4">Nenhuma transação pendente.</p>
                            ) : (
                                upcomingTransactions.map((t) => (
                                    <div key={t.id} className="flex items-center justify-between py-2 border-b last:border-0">
                                        ...
                                        <div className={`font-medium ${t.type === 'receivable' ? 'text-emerald-600' : 'text-red-600'}`}>
                                            {t.type === 'receivable' ? '+' : '-'}
