    ...
    const loadData = async () => {
      try {
        ...
        if (batchData.status !== "pending_authorization") {
          setStatus("error");
          setErrorMessage(
            `Este lote não está aguardando autorização (status: ${batchData.status}).`,
          );
          return;
        }
      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded p-4 flex gap-3 text-amber-800 text-sm">
            ...
            <p>
              Ao confirmar, você está autorizando o processamento deste lote no
              sistema bancário. Esta ação não pode ser desfeita.
            </p>
          </div>
        </CardContent>
