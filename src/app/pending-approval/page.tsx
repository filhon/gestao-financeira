...
export default function PendingApprovalPage() {
      ...
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          const effectiveStatus =
            data.status || (data.active ? "active" : "pending_approval");
  ...
  useEffect(() => {
    if (user) {
      const effectiveStatus =
        user.status || (user.active ? "active" : "pending_approval");
      if (effectiveStatus === "active") {
        Cookies.set("user_status", "active", { expires: 1 / 24 });
        router.push("/dashboard");
      }
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 pb-4">
          ...
          <CardDescription className="text-center">
            Olá, {user?.displayName?.split(" ")[0]}! Sua solicitação foi
            enviada.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-md text-sm text-blue-700 dark:text-blue-300 text-center">
            Esta página será atualizada automaticamente quando sua conta for
            aprovada.
          </div>
