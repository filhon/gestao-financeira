...
export function usePermissions(): Permissions {
  ...
  const companyRole = (user &&
    selectedCompany?.id &&
    user.companyRoles?.[selectedCompany.id]) as UserRole | undefined;

  const isGlobalAdmin = globalRole === "admin";
  const effectiveRole: UserRole | null = isGlobalAdmin
    ? "admin"
    : companyRole || null;
