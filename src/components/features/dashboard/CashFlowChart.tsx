...
import { ProjectedCashFlowData, dashboardService } from "@/lib/services/dashboardService";
...
export function CashFlowChart() {
    ...
    const [data, setData] = useState<ProjectedCashFlowData[]>([]);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            setIsLoading(true);
            try {
                const result = await dashboardService.getProjectedCashFlow(selectedCompany.id, mode);
                setData(result);
            } catch (error) {
                console.error("Error loading projected cash flow:", error);
            } finally {
                setIsLoading(false);
            }
        };
                        ...
                        <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                            ...
                                            <div className="rounded-lg border bg-popover p-3 shadow-md">
                                                ...
                                                <div className="mt-2 space-y-1">
                                                    <p className="text-sm">
                                                        ...
                                                        <span className={data.balance >= 0 ? 'text-emerald-600 font-medium' : 'text-red-600 font-medium'}>
                            ...
                            {hasNegative && (
                                <ReferenceLine
                                    ...
                                    stroke="#ef4444"
                                    strokeWidth={2}
                                    label={{ value: 'Saldo Zero', position: 'right', fill: '#ef4444', fontSize: 10 }}
                                />
                            )}
                            <Area
                                type="monotone"
