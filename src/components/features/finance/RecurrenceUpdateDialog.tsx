...
export function RecurrenceUpdateDialog({
  ...
  const remainingCount = isInstallment
    ? installmentInfo.total - installmentInfo.current + 1
    : 0;
      ...
      <AlertDialogContent>
          ...
          <AlertDialogDescription>
            {isInstallment ? (
              <>
                Esta transação é a{" "}
                <strong>
                  parcela {installmentInfo.current} de {installmentInfo.total}
                </strong>
        ...
        <AlertDialogFooter className="flex-col sm:flex-row gap-2">
          ...
          <Button onClick={() => onConfirm("series")}>
            Esta e as próximas{" "}
            {isInstallment && remainingCount > 1 ? `(${remainingCount})` : ""}
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
