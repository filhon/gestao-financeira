...
export function BatchApprovalDialog({
    ...
    const handleSaveAmount = () => {
        ...
        if (currentEdit) {
            setEdits(new Map(edits.set(editingAmountId, {
                ...currentEdit,
                adjustedAmount: newAmount !== currentEdit.originalAmount ? newAmount : undefined,
            })));
        }
    ...
    const handleConfirmReject = async () => {
        ...
        try {
            await paymentBatchService.rejectTransaction(batch.id, rejectingId, rejectReason, userId);
        ...
        try {
            ...
            edits.forEach((edit) => {
                if (edit.adjustedAmount !== undefined) {
                    adjustments.push({ id: edit.id, adjustedAmount: edit.adjustedAmount });
                }
            });
                                                    ...
                                                    {editingAmountId === t.id ? (
                                                        <div className="flex items-center gap-1 justify-end">
                                                            <Input
                                                                ...
                                                                autoFocus
                                                            />
                                                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={handleSaveAmount}>
                                                                <Check className="h-4 w-4 text-green-600" />
                                                            </Button>
                                                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setEditingAmountId(null)}>
                                                                <X className="h-4 w-4" />
                                                            </Button>
                                                        </div>
                                                    ) : (
