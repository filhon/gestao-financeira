...
export function getActionSummary(
  ...
  switch (action) {
    ...
    case "pay":
      return `${entityLabel} pago(a)`;

    case "authorize":
      return `${entityLabel} autorizado(a)`;

    case "release":
      return `${entityLabel} liberado(a)`;
...
export function getEntityLink(entity: string, entityId: string): string | null {
  switch (entity) {
    case "transaction":
      // Link para a página financeira; sem página de detalhe individual
      return entityId ? `/financeiro` : null;
    case "cost_center":
