...
import { startOfMonth, endOfMonth, subMonths, format, addDays, startOfYear, endOfYear, isBefore, isAfter, isSameDay } from "date-fns";
...
export const dashboardService = {
    ...
    getProjectedCashFlow: async (companyId: string, mode: '30days' | 'year' = '30days'): Promise<ProjectedCashFlowData[]> => {
        ...
        transactions.forEach(t => {
            if (t.status === 'paid' && isBefore(t.dueDate, startDate)) {
                const amount = Number(t.amount) || 0;
                if (t.type === 'receivable') {
        ...
        while (!isAfter(currentDate, endDate)) {
            ...
            transactions.forEach(t => {
                ...
                const isInPeriod = mode === 'year'
                    ? (!isBefore(dueDate, currentDate) && isBefore(dueDate, nextDate))
    ...
    getBudgetProgressByCostCenter: async (companyId: string): Promise<BudgetProgressData[]> => {
        const now = new Date();
        ...
        const yearTxQuery = query(
            ...
            where("companyId", "==", companyId),
            where("type", "==", "payable"),
            where("dueDate", ">=", Timestamp.fromDate(yearStart)),
            where("dueDate", "<", Timestamp.fromDate(currentMonthStart))
        );
        const yearTxSnapshot = await getDocs(yearTxQuery);
        const yearTransactions = yearTxSnapshot.docs.map(doc => doc.data() as Transaction);
        ...
        yearTransactions.forEach(t => {
            ...
            if (t.costCenterAllocation && t.costCenterAllocation.length > 0) {
                t.costCenterAllocation.forEach(alloc => {
                    ...
                    ytdExpensesByCC.set(alloc.costCenterId, current + (alloc.amount || 0));
                });
            } else if (t.costCenterId) {
                const current = ytdExpensesByCC.get(t.costCenterId) || 0;
                ytdExpensesByCC.set(t.costCenterId, current + Number(t.amount));
            }
        });
        ...
        const txQuery = query(
            ...
            where("type", "==", "payable"),
            where("dueDate", ">=", Timestamp.fromDate(currentMonthStart)),
            where("dueDate", "<=", Timestamp.fromDate(currentMonthEnd))
        );
        ...
        transactions.forEach(t => {
            ...
            if (t.costCenterAllocation && t.costCenterAllocation.length > 0) {
                t.costCenterAllocation.forEach(alloc => {
                    ...
                    currentMonthExpensesByCC.set(alloc.costCenterId, current + (alloc.amount || 0));
                });
            } else if (t.costCenterId) {
                const current = currentMonthExpensesByCC.get(t.costCenterId) || 0;
                currentMonthExpensesByCC.set(t.costCenterId, current + Number(t.amount));
            }
        });
        ...
        const result: BudgetProgressData[] = costCenters.map(cc => {
            ...
            const adjustedMonthlyBudget = remainingMonths > 0 ? remainingBudget / remainingMonths : 0;

            const percentage = adjustedMonthlyBudget > 0
