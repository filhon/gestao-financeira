...
export const userService = {
  ...
  /**
   * Remove acesso de um usuário a uma empresa específica
   */
  revokeAccess: async (
    uid: string,
    companyId: string,
    ...
    const currentDoc = await getDoc(docRef);
    const oldRole = currentDoc.exists()
      ? currentDoc.data()?.companyRoles?.[companyId]
      : undefined;

    await updateDoc(docRef, {
      [`companyRoles.${companyId}`]: deleteField(),
      updatedAt: serverTimestamp(),
    });
    ...
    await auditService.log({
      ...
      entityId: uid,
      details: {
        changes: [{ field: "role", oldValue: oldRole, newValue: null }],
        companyId,
      },
    });
  },
