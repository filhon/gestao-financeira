...
export const feedbackService = {
  ...
  create: async (data: CreateFeedbackData): Promise<string> => {
    ...
    await feedbackService.notifyAdminsOfNewFeedback(
      data.userName,
      data.userEmail,
      data.type,
      data.title,
  ...
  /**
   * Notify global admins about new feedback
   * Sends both in-app notification and email via Resend
   */
  notifyAdminsOfNewFeedback: async (
    ...
    if (adminEmails.length > 0) {
      try {
        await emailService.sendFeedbackNotification(adminEmails, {
          userName,
          userEmail,
          feedbackType,
          title,
        ...
        console.error("Error sending feedback email notifications:", error);
        // Don't throw - notifications were already sent
      }
    }
  },
