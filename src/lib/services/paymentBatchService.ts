...
const convertDates = (data: DocumentData): PaymentBatch => {
    return {
        ...
        sentForAuthorizationAt: (data.sentForAuthorizationAt as Timestamp)?.toDate(),
        authorizedAt: (data.authorizedAt as Timestamp)?.toDate(),
        paidAt: (data.paidAt as Timestamp)?.toDate(),
        approvalTokenExpiresAt: (data.approvalTokenExpiresAt as Timestamp)?.toDate(),
    } as PaymentBatch;
};
...
export const paymentBatchService = {
    ...
    rejectTransaction: async (
        ...
        const newTransactionIds = batchData.transactionIds.filter(id => id !== transactionId);
        const rejectedIds = [...(batchData.rejectedTransactionIds || []), transactionId];
    ...
    getByApprovalToken: async (token: string): Promise<PaymentBatch | null> => {
        ...
        const batch = convertDates({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() });
        
        // Check token expiration
        if (batch.approvalTokenExpiresAt && batch.approvalTokenExpiresAt < new Date()) {
            throw new Error("Link expirado. Solicite um novo link ao gestor financeiro.");
        }
    ...
    approveByToken: async (
        ...
        if (paymentBatch.status !== 'pending_approval') {
            throw new Error(`Este lote não está aguardando aprovação (status: ${paymentBatch.status}).`);
        }
    ...
    authorizeByToken: async (token: string): Promise<void> => {
        ...
        if (paymentBatch.status !== 'pending_authorization') {
            throw new Error(`Este lote não está aguardando autorização (status: ${paymentBatch.status}).`);
        }
