...
export const importService = {
    ...
    validateRow: (row: ImportedRow, costCenters: CostCenter[]): ImportedRow => {
        ...
        if (amount === null) {
            errors.push({ field: 'amount', message: 'Valor inválido ou menor que zero' });
        }
        ...
        if (!row.supplierOrClient || row.supplierOrClient.toString().trim().length < 2) {
            errors.push({ field: 'supplierOrClient', message: 'Fornecedor/Cliente é obrigatório' });
        }
        ...
        if (row.costCenterCode && !costCenterId) {
            ...
            const found = costCenters.find(cc => 
                cc.code.toUpperCase() === code || 
                cc.name.toUpperCase() === code
            );
            if (found) {
                costCenterId = found.id;
            } else {
                warnings.push({ field: 'costCenterCode', message: `Centro de custo "${row.costCenterCode}" não encontrado` });
            }
        }
        
        // Check if cost center is missing (warning, not error - can be bulk assigned)
        if (!costCenterId && !row.costCenterCode) {
            warnings.push({ field: 'costCenterCode', message: 'Centro de custo não informado' });
        }
        ...
        if (row.paymentMethod) {
            ...
            if (!paymentMethod) {
                warnings.push({ field: 'paymentMethod', message: `Forma de pagamento "${row.paymentMethod}" não reconhecida` });
            }
        }
     ...
     * Validate all rows
     */
    validateRows: (rows: ImportedRow[], costCenters: CostCenter[]): ImportedRow[] => {
     ...
     */
    applyDefaultCostCenter: (rows: ImportedRow[], defaultCostCenterId: string): ImportedRow[] => {
        return rows.map(row => {
            if (!row.costCenterId) {
                const updatedRow = { 
                    ...row, 
                    ...
                    warnings: row.warnings.filter(w => w.field !== 'costCenterCode' || w.message !== 'Centro de custo não informado')
                };
                return updatedRow;
            }
            return row;
     ...
     */
    applyCostCenterToSelected: (rows: ImportedRow[], selectedIds: Set<string>, costCenterId: string): ImportedRow[] => {
        return rows.map(row => {
