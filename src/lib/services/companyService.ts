...
export const companyService = {
  ...
  update: async (
    ...
    // Regenera campos normalizados se nome ou CNPJ foram alterados
    const normalizedFields: Record<string, string | null> = {};
    if (data.name !== undefined) {
      normalizedFields.normalizedName = normalizeName(data.name);
    }
    if (data.cnpj !== undefined) {
      normalizedFields.normalizedCnpj = data.cnpj
        ? normalizeCnpj(data.cnpj)
        : null;
    }
